...
const DanhSachPhieuChamDiemNhaCungCap = () => {
  ...
  const { data, refetch, isFetching } = useQuery({
    ...
    queryFn: () => chatLuongHangHoaApi.getAll(params),
    select: (response) =>
  ...
  const onAdd = () => history.push("/goods-quality/create");

  const onEdit = () => {
    const id = data.find((e) => e.check)?.id;
    history.push(`/goods-quality/edit/${id}`);
  };
  ...
  const onRemove = async () => {
    ...
    if (allow) {
      try {
        await chatLuongHangHoaApi.remove(selected?.[0]?.id);
        refetch();
        fireSuccess();
      } catch (error) {
        fireError();
